...
class LimerickDetector:
    ...
    def is_limerick(self, text):
        ...
        #tk_list = [
        #    [x for x in self.apostrophe_tokenize(s) if x not in punctuation]
        #    for s in text.decode('unicode_escape').lower().split('\n')
        #    if len(s.strip()) != 0]
        ...
        # for s in text.decode('unicode_escape').lower().split('\n'):
            # if len(s.strip()) != 0:
                # print len(self.apostrophe_tokenize(s)), self.apostrophe_tokenize(s)
    ...
    def apostrophe_tokenize(self, sen):

        tokens = [x for x in word_tokenize(sen) if x not in punctuation]
        # print 'old', tokens
        new_tk = []
        # for tokens generated by nltk word_tokenize function
        # if there's any token containing apostrophe s
        # then concatenate with previous token
        # based on grammatical rule in English
        ...
        while len(tokens) != 0:
            ...
            if curr.find("'") != -1:
                ...
                temp = (prev+curr)
                if sen.find(temp) != -1:
                    # if newly generated token is found in original sentence
                    # we corrected apostrophe error. put it in list
                    new_tk.insert(0, temp)
                else:
                    # otherwise, we made something wrong
                    # put two tokens in list separately
                    new_tk.insert(0, prev)
                    new_tk.insert(0, curr)
            else:
                new_tk.insert(0, curr)
        # print 'new', new_tk
    ...
    def guess_syllables(self, w):
        # guess number of syllables by counting number of vowels in word
        # y is also considered to act like vowels
        return sum([1 for x in w if x in ['a', 'i', 'e', 'o', 'u', 'y']])
